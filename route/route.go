...
func (p *ParsedRoute) SummaryShort(ca *x509.CertPool) (string, error) {
	if len(p.Hop) == 0 {
...
func (p *ParsedRoute) Verify(ca *x509.CertPool) error {
	_, hopIdentities, err := p.DecodeHops(ca)
	if err != nil {
		return err
	}
...
func (p *ParsedRoute) DecodeHops(ca *x509.CertPool) ([]*Route_Hop, []*identity.ParsedIdentity, error) {
	if p.routeHops != nil {
...
func (p *ParsedRoute) AddHop(ca *x509.CertPool, hop *Route_Hop, pkey *rsa.PrivateKey) error {
	if p.routeHops == nil {
...
func (r *Route) DecodeHops(caCerts *x509.CertPool) ([]*Route_Hop, []*identity.ParsedIdentity, error) {
	...
	for i, bin := range r.Hop {
		...
		if err := pident.VerifyMessage(caCerts, bin); err != nil {
			return nil, nil, err
		}
		result[i] = h
