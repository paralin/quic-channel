...
func socket(family, socktype, protocol int) (fd int, err error) {
	...
	if err = syscall.SetsockoptInt(fd, syscall.SOL_SOCKET, soBroadcast, 1); err != nil {
		syscall.Close(fd)
		return -1, err
	}
...
func listen(netw string, addr net.Addr) (fd int, err error) {
	var (
		family   int
		socktype int
		protocol int
...
func listenStream(netw string, addr net.Addr) (l net.Listener, err error) {
	var (
		file *os.File
	)
...
func listenPacket(netw string, addr net.Addr) (p net.PacketConn, err error) {
	var (
		file *os.File
	)
...
func listenUDP(netw string, addr net.Addr) (c net.Conn, err error) {
	var (
		file *os.File
	)
